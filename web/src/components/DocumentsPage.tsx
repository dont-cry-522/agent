...
export default function DocumentsPage({ onMenuClick }: DocumentsPageProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-8">
        <h2 className="hidden sm:block text-xl font-semibold text-gray-800 mb-6">知识库文档</h2>

        {stats && (
          <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
            <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4">
              <p className="text-xl sm:text-2xl font-bold text-gray-800">{stats.document_count}</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">文档总数</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4">
              <p className="text-xl sm:text-2xl font-bold text-gray-800">{stats.chunk_count}</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Chunk 总数</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4">
              <p className="text-xl sm:text-2xl font-bold text-gray-800">{formatSize(stats.total_size)}</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">总大小</p>
            </div>
          </div>
        )}
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-6 sm:p-10 text-center mb-6 sm:mb-8 transition-colors cursor-pointer ${
        ...
        >
          ...
          {uploading ? (
            ...
            <>
              <svg className="w-8 h-8 sm:w-10 sm:h-10 text-gray-400 mx-auto mb-2 sm:mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
              <p className="text-sm text-gray-600 font-medium">拖拽文件到此处上传</p>
              <p className="text-xs text-gray-400 mt-1">或点击选择文件 · 支持 MD/PDF/DOCX/TXT/HTML</p>
            </>
          )}
        </div>
        ...
        {documents.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
            <p className="text-gray-400 text-sm">暂无文档，上传文件开始构建知识库</p>
          </div>
        ) : (
          <>
            {/* 移动端卡片列表 */}
            <div className="sm:hidden space-y-3">
              {documents.map((doc) => (
                <div key={doc.id} className="bg-white border border-gray-200 rounded-xl p-3.5">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 text-xs text-gray-500">
                        <span className="bg-gray-100 px-1.5 py-0.5 rounded">{doc.format}</span>
                        <span>{formatSize(doc.file_size)}</span>
                        <span className="bg-violet-50 text-violet-700 px-1.5 py-0.5 rounded font-medium">{doc.chunk_count} chunks</span>
                      </div>
                      <p className="text-xs text-gray-400 mt-1">{doc.created_at}</p>
                    </div>
                    <button
                      onClick={() => handleDelete(doc)}
                      className="text-xs text-red-500 hover:text-red-700 transition-colors cursor-pointer shrink-0"
                    >
                      删除
                    </button>
                  </div>
